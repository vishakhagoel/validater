...
def check_dependencies(approvers, directory_path):
    ...
    for dependency in dependency_map.get(directory_path, []):
...
def validate_approval(approvers, files_changed):
    for file in files_changed:
        ...
        if check_owners(approvers, directory_path) and check_dependencies(approvers, directory_path):
            return APPROVED
    return NOT_APPROVED


#  parses the user input and calls validate_approval()
